...
//"WORLD: Hello!!!"
=========================
//подвыражения (группировка)
!к подвыражению можно обратиться по индексу.
!подвыражения используются для задания альтернатив.
--------------------------
Cледующее выражение соответствует строкам «+7 902», «8(902)» и еще множеству вариантов: /^(\+7|8)\s*(\d{3}|\(\d{3}\))/
--------------------------
 «заглядывание вперед» — это нечасто применяемая на практике техника позволяет проверить соответствие подвыражению, не смещая позицию поиска и не запоминая найденное соответствие. Синтаксис используется следующий "(?=pattern)". 
